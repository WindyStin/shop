...
public class OrderDao {
    ...
    public List<Order> selectOrderList(int status, int pageNumber, int pageSize) throws SQLException {
        QueryRunner r = new QueryRunner(DataSourceUtils.getDataSource());
        if(status==0) {
            String sql = "select o.id,o.total,o.amount,o.status,o.paytype,o.name,o.phone,o.address,o.datetime,u.username,u.email from `order` o,user u where o.user_id=u.id order by o.datetime desc limit ?,?";
            return r.query(sql, new BeanListHandler<Order>(Order.class), (pageNumber-1)*pageSize,pageSize );
        }else {
            String sql = "select o.id,o.total,o.amount,o.status,o.paytype,o.name,o.phone,o.address,o.datetime,u.username,u.email from `order` o,user u where o.user_id=u.id and o.status=? order by o.datetime desc limit ?,?";
    ...
    // 统计订单项按商品分组的总数量和总价
    public List<Map<String, Object>> getOrderSummary() throws SQLException {
        QueryRunner runner = new QueryRunner(DataSourceUtils.getDataSource());
        // 修改为按商品名称分组，并统计总数量和总价
        String sql = "SELECT g.name AS goods_name, SUM(oi.amount) AS total_amount, " +
                "SUM(oi.amount * oi.price) AS total_price " +
                "FROM orderitem oi " +
    ...
    // 根据订单ID查询订单的地址和其他相关信息
    public Map<String, Object> getOrderDetailsById(int orderId) throws SQLException {
        QueryRunner runner = new QueryRunner(DataSourceUtils.getDataSource());
        String sql = "SELECT o.address, o.id, o.total, u.username, u.email " +
                "FROM `order` o " +
